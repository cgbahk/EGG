...
if __name__ == "__main__":
    ...
    interaction_file = glob.glob(
        "./data/training_inter/imagenet*None*['vit', 'inception*"
    )[0]
    ...
    train_reps, val_reps = train_test_split(
        train_reps, test_size=0.25, random_state=42
    )  # 0.25 * 0.8 = 0.2
    ...
    print(
        f"Input representation size: {input_size}, expansion factor: {expansion_factor}, Hidden SAE size: {hidden_size}"
    )
    ...
    train_loader = torch.utils.data.DataLoader(
        train_reps, batch_size=batch_size, shuffle=True
    )
    val_loader = torch.utils.data.DataLoader(
        val_reps, batch_size=batch_size, shuffle=False
    )
    for epoch in range(epochs):
        ...
        # Log metrics
        wandb.log(
            {"epoch": epoch, "training_loss": loss.item(), "validation_loss": val_loss}
        )

        print(
            f"Epoch {epoch}, Training loss: {loss.item()}, Validation loss: {val_loss}"
        )
