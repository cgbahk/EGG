...
def main():
    ...
    data_paths = [
        None,
        None,
        "/projects/colt/imagenet21k_resized/imagenet21k_train/",
        "/projects/colt/ILSVRC2012/ILSVRC2012_img_val/",
        "/projects/colt/ILSVRC2012/ILSVRC2012_img_val/",
        "/home/mmahaut/projects/emecom/data",
        None,
        "/projects/colt/celebA",
    ]
    _sc = False
    for model_name in model_names:
        ...
        for dataset_name, dataset_path in zip(dataset_names, data_paths):
            ...
            for batch in tqdm.tqdm(
                dl,
                desc=f"Processing {dataset_name} with {model_name}",
                total=n_collected_images // batch_size - 1,
            ):
        ...
        for i, model1 in enumerate(model_names):
            for j, model2 in enumerate(model_names):
                if i < j:
                    agreement[i, j] = (
                        (
                            torch.tensor(outputs[model1][dataset])
                            == torch.tensor(outputs[model2][dataset])
                        )
                        .float()
                        .mean()
                    )
            ...
            for model_name2 in model_names:
                if model_name1 != model_name2:
                    ...
                    for i in range(0, len(outputs[model_name1][dataset]), batch_size):
                        score = 0
                        msgs1 = torch.tensor(
                            outputs[model_name1][dataset][i : i + batch_size]
                        )
                        msgs2 = torch.tensor(
                            outputs[model_name2][dataset][i : i + batch_size]
                        )
                        mask = msgs1 == msgs2
                        score += (
                            (torch.rand((~mask).sum()) < 1 / batch_size).float().sum()
                        )
