...
if __name__ == "__main__":
    ...
    for i, (chunk_imgs_i, _) in enumerate(
        tqdm(train_dataloader, desc="Calculating cosine similarity", leave=True)
    ):
        chunk_imgs_i = chunk_imgs_i.view(chunk_imgs_i.size(0), -1).to(device)
        if i == len(train_dataloader) - 1:
            chunk_imgs_i = torch.cat(
                (
                    chunk_imgs_i,
                    torch.zeros(
                        (batch_size - chunk_imgs_i.size(0), chunk_imgs_i.size(1))
                    ).to(device),
                )
            )

        for j, (chunk_imgs_j, _) in enumerate(
            tqdm(train_dataloader, desc=f"{i}th horizontal sweep", leave=False)
        ):
            chunk_imgs_j = chunk_imgs_j.view(chunk_imgs_j.size(0), -1).to(device)
            # pad the last chunk
            if j == len(train_dataloader) - 1:
                chunk_imgs_j = torch.cat(
                    (
                        chunk_imgs_j,
                        torch.zeros(
                            (batch_size - chunk_imgs_j.size(0), chunk_imgs_j.size(1))
                        ).to(device),
                    )
                )

            # Calculate pairwise cosine similarity for the chunk
            chunk_cos_sim_matrix = torch.nn.functional.cosine_similarity(
                chunk_imgs_i, chunk_imgs_j, dim=1
            )
            ...
            ] = chunk_cos_sim_matrix[0].cpu().numpy()
